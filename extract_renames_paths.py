...
from parsers.pd.pdparser_path import main as pdparser
import sys
import json
...
def get_revisions_and_run_parser(cwd, project_name, main_branch, debug=False):
    ...
    for f in filenames:
        ...
        for c in sorted(all_sha_dates, key=all_sha_dates.get): # start oldest to newest
            ...
            new_txt_file_name = name3.stdout.decode().strip()
            
            with open("/data/play/aislam4/thesis/pd_parsed/paths_revisions/" + project_name + "/" + new_txt_file_name + ".txt", "w") as outfile:
